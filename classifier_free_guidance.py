...
class UNetCond(nn.Module):
    ...
    def forward(self, x, timesteps, labels=None):
        ...
        if labels is not None:
            ...
            if torch.equal(t, t_before_addition):
                self.ignore_cnt += 1
...
for epoch in range(epochs):
    ...
    print(f'Epoch {epoch} | Loss: {loss_avg} | ignored condition cnt: {model.get_ignore_cnt()}')
...
# generate samples
images, labels = diffuser.sample(model)
print(f'ignored condition cnt: {model.get_ignore_cnt()}')
show_images(images, labels)
# torch.save(model.to('cpu').state_dict(), 'classifier_free_guidance.pth')
